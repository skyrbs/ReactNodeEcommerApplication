...
import {updateCustomer, customerdetail, getSalesusers} from "./apiSalesAdmin";
...
const UpdateCustomersDetails = props => {
    ...
    const redirectUser = () => {
        if (redirectToProfile) {
            if (!error) {
                return <Redirect to="/salesadmin/salesusers" />;
            }
        }
    };
    ...
    const goBack = () => (
        <div className="mt-3 mb-3 bradcrum">
            ...
            <Link to="/salesadmin/salesusers" className="text-warning">
                Manage Sales Person
            </Link>
        </div>
    );
